...
rgb.img <- pacu::pa_get_rgb(s2.files, aoi = area.of.interest)
pa_plot(rgb.img)
```
...
in a time series format, we can visualize the data by
plotting the minimum, maximum, and mean NDVI in our area
of interest.

```{r vis-ndvi-stats, fig.width=7, fig.height = 5}
with(as.data.frame(ndvi.statistics), {
  plot(time, ndvi.min, col = 'steelblue', ylim = c(0, 1), ylab = 'NDVI')
  points(time, ndvi.max, col = 'tomato1')
  points(time, ndvi.mean, col = 'navyblue')
  legend('topleft', pch = 1, col = c('steelblue', 'tomato1', 'navyblue'), legend = c('min', 'max', 'mean'))
})
