...
#' @description Create an interpolated yield object from raw data.
#' @name pa_yield
#' @rdname pa_yield
...
#'   the variable.
#' @param grid an `sf` or `pa_trial` object containing the prediction grid. If
#'   the user is processing yield data from a research trial, the trial layout
#'   can be supplied here. If `formula` includes predictors, those predictors
#'   should also be present in the object supplied to `grid`.
...
#'   \link[gstat]{krigeTg} to perform kriging on the log scale and back-transform
#'   predictions to the original scale. For now, this is only relevant when
#'   `smooth.method = "krige"`, and `formula` should be \sQuote{z ~ 1}.
#' @param lbs.per.bushel a numeric value representing the
#'   number of pounds in a bushel (e.g., 60 for soybean and
#'   56 for corn). This argument can be omitted when the
...
#'   units of the grid. This is intended to prevent
...
pa_yield <- function(input,
  ...
  if (algorithm == 'ritas') {
    ...
    if(remove.crossed.polygons) {
      ...
      grid <- sf::st_transform(grid, sf::st_crs(input))
      cpi <- sf::st_covered_by(adj.pols, sf::st_buffer(grid, 0.01))
