...
#' @description Retrieve weather data from NASA POWER or the Iowa Environmental
#'   Mesonet using the `apsimx` package.
#' @param aoi an `sf` object.
#' @param source weather data source. `iem` uses the Iowa Environmental Mesonet
#'   and `power` uses NASA POWER. Defaults to `iem`.
#' @param start.date first day to retrieve weather data. Format should be
#'   `\%Y-\%m-\%d`.
#' @param end.date last day to retrieve weather data. Format should be
#'   `\%Y-\%m-\%d`.
