...
#' @description Register Copernicus Data Space Ecosystem credentials in the R
#'   environment.
#' @name pa_initialize_dataspace
#' @rdname pa_initialize_dataspace
#' @param username username used to authenticate HTTP requests.
#' @param password password used to authenticate HTTP requests.
#' @param verbose logical; whether to print information about this operation.
#' @details `pa_initialize_dataspace()` stores the username and password in the
#'   user's R environment file. Other functions that require authentication look
#'   for these credentials there. Do not share your R environment with others,
#'   because they will be able to read your username and password. You can
...
#' @description Browse satellite products available from the Copernicus Data
#'   Space Ecosystem.
#' @name pa_browse_dataspace
#' @rdname pa_browse_dataspace
#' @param aoi `sf` object used to filter satellite products.
#' @param start.date beginning of the time window used to filter satellite
#'   products. The date format should be `\%Y-\%m-\%d`.
#' @param end.date end of the time window used to filter satellite products. The
#'   date format should be `\%Y-\%m-\%d`.
#' @param max.cloud.cover maximum cloud cover. Values should be between 0 and
#'   100. Images with cloud cover estimates greater than this value are removed
#'   from the results.
#' @param collection.name product collection to filter. Currently, only
#'   `SENTINEL-2` is supported.
#' @param product.name partial product name used to filter results. Currently,
#'   only `MSIL2A` is supported.
#' @param max.results maximum number of results to return.
#' @details `pa_browse_dataspace()` communicates with the Data Space API through
#'   HTTP requests and returns the products matching the requested filters.
#' @return A data frame of entries available for download.
#' @author Caio dos Santos and Fernando Miguez
#' @export
#' @examples
#' \dontrun{
...
#' @description Download satellite products from the Copernicus Data Space
#'   Ecosystem.
#' @name pa_download_dataspace
#' @rdname pa_download_dataspace
#' @param x object of class \sQuote{dslist}.
#' @param dir.path directory where files will be saved.
#' @param aoi `NULL` or an `sf` object. If an area of interest is provided, the
#'   downloaded zip files are cropped to that extent to reduce storage use.
#' @param verbose logical; whether to display information about progress.
#' @details `pa_download_dataspace()` uses the object returned by
#'   `pa_browse_dataspace()` to download products from Copernicus Data Space. The
#'   `aoi` argument is optional, but it can substantially reduce storage needs.
#' @return A list of products that could not be downloaded.
#' @author Caio dos Santos and Fernando Miguez
#' @export
#' @examples
#' \dontrun{
...
pa_download_dataspace <- function(x,
  ...
  if("" %in% c(Sys.getenv('DATASPACE_USERNAME'), Sys.getenv('DATASPACE_PASSWORD')))
    stop('Data Space username or password is not registered in the R environment. Use pa_initialize_dataspace() to register credentials.')
