...
#' @description Request vegetation index statistics from the Data Space
#'   Statistics API.
#' @name pa_get_vi_stats
#' @rdname pa_get_vi_stats
#' @param aoi `sf` object used to define the area of interest.
#' @param start.date beginning of the time window used to filter satellite
#'   products. Date format should be `\%Y-\%m-\%d`.
#' @param end.date end of the time window used to filter satellite products.
#'   Date format should be `\%Y-\%m-\%d`.
#' @param collection satellite collection. Currently only `sentinel-2-l2a` is
#'   supported.
#' @param vegetation.index vegetation index to request from Data Space.
#' @param agg.time aggregation interval for the statistics request.
#' @param by.feature logical; whether statistics should be retrieved separately
#'   for each polygon when multiple polygons are supplied in `aoi`.
#' @details `pa_get_vi_stats()` communicates with the Data Space Statistics API
#'   through HTTP requests and returns area-level vegetation index summaries for
#'   the requested area and time range.
...
pa_get_vi_stats<- function(aoi,
  ...
  if("" %in% c(Sys.getenv('DATASPACE_CLIENTID'), Sys.getenv('DATASPACE_CLIENTSECRET')))
    stop('Data Space OAuth client ID or client secret is not registered in the R environment. Use pa_initialize_oauth() to register your credentials.')
...
#' @description Register OAuth 2.0 credentials in the R environment.
#' @name pa_initialize_oauth
#' @rdname pa_initialize_oauth
#' @param client_id client ID used to authenticate HTTP requests.
#' @param client_secret client secret used to authenticate HTTP requests.
#' @details `pa_initialize_oauth()` stores the client ID and client secret in
#'   the user's R environment file. Other functions that rely on authentication
#'   look for these credentials there. Do not share your R environment with
#'   others, because they will be able to read your credentials. You can
#'   register at \url{https://dataspace.copernicus.eu/news}. Please see this
#'   section for instructions on creating an OAuth 2.0 client:
